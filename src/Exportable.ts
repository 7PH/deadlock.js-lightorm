    ...
    public import(data: string | object): this {
        ...
        for (let key in json) {
            ...
            if (! json.hasOwnProperty(key))
                throw new Error("Trying to import a key which does not exist on Exportable: " + key);

            if (! ExportableHandler.isExportable(this, key))
                throw new Error("Key " + key + " is not registered as exportable");
